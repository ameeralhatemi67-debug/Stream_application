// Test-only fixtures: the five sample AlSharqia broadcasters that used to be
// compiled into lib/ and loaded into AppProvider at construction. The app now
// starts with no streamers and fills the list from the backend (P2 truthful
// data); tests that need a populated feed seed these explicitly with
// seedStreamerFixtures().
...
/// The five sample broadcasters (all initially offline).
...
/// Loads the five sample broadcasters into [provider], the way the app used to
/// start before the fixtures were removed from lib/.
void seedStreamerFixtures(AppProvider provider) {
