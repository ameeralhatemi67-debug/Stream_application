...
```
+-------------------------------------------------------------------------------------------------+
|                                     LOCAL INGESTION STAGE                                       |
|  +--------------------+       RTMP/SRT       +--------------------+      Dedicated Fiber        |
|  | PTZ Video Cameras  | ===================> | Hardware Encoder   | =========================+  |
|  | (Al Khobar Venue)  |                      | (Local H.264/AAC)  |                          |  |
|  +--------------------+                      +--------------------+                          |  |
+----------------------------------------------------------------------------------------------|--+
                                                                                               |
+----------------------------------------------------------------------------------------------|--+
|                                     CLOUD CORE PROCESSING STAGE                              |
|  +-------------------------+      ABR Transcoding      +----------------------------------+  |  |
|  | AWS Riyadh Region       | ========================> | Amazon IVS Channel               | <+  |
|  | (me-central-2 Ingest)   |                           | (Multi-Bitrate HLS Generation)   |     |
|  +-------------------------+                           +----------------------------------+     |
|                                                                    ||                           |
|                                                                    || Archive to VOD            |
|                                                                    \/                           |
|  +-------------------------+                           +----------------------------------+     |
|  | Amazon S3 Bucket        | <======================== | Automated Recording              |     |
|  | (me-central-2 Storage)  |                           | Configuration (Active Logs)      |     |
|  +-------------------------+                           +----------------------------------+     |
+----------------------------------------------------------------------------------------------|--+
                                                                                               |
+----------------------------------------------------------------------------------------------|--+
|                                     EDGE DISTRIBUTION STAGE                                  |
|  +-------------------------+      Cached Segments      +----------------------------------+  |  |
|  | Mobile Playback Client  | <======================== | CloudFront Jeddah Edge Node      | <+  |
|  | (Low-Latency Player)    |                           | (Local Egress Delivery)          |     |
|  +-------------------------+                           +----------------------------------+     |
+-------------------------------------------------------------------------------------------------+
```
...
```
                  ...
                  | - Chat room token authorization   |
                  +-----------------------------------+
                                    ^
                                    | WebSocket (Bidirectional)
                                    v
+------------------+      Timed Metadata (In-Band)     +------------------+
| Playback Client  | <================================ | AWS IVS Put      |
| (User Device)    |                                   | Metadata API     |
+------------------+                                   +------------------+
                                                                ^
                                                                | HTTP POST
                                                       +------------------+
...
```
+--------------------------+
| Existing Video Logs      | (Legacy MP4/MOV formats)
+--------------------------+
             |
             | Upload to Cloud
             v
+--------------------------+
| Amazon S3 Ingest Bucket  | (me-central-2 Riyadh)
+--------------------------+
             |
             | Trigger Transcoding Job
             v
+--------------------------+
| AWS Elemental            | - Generates multi-bitrate ABR ladder (1080p to 360p)
| MediaConvert             | - Packages into HLS (.m3u8) index files
+--------------------------+
             |
             | Write Output Assets
             v
+--------------------------+
| Amazon S3 VOD Bucket     | ====> [ CloudFront Edge Nodes ] ====> Users
+--------------------------+
             |
             | Register Metadata
             v
+--------------------------+
| Amazon DynamoDB          | - Stores Video ID, Title, Date, Lecturer
| (Metadata Index)         | - Links to CloudFront HLS URL
+--------------------------+
```
...
```
                +-------------------------------------------+
                |     Saudi Regulatory Compliance Core      |
                +-------------------------------------------+
                               /      |      \
                              /       |       \
                             /        |        \
                            v         v         v
+----------------------------+  +--------------+  +----------------------------+
| GAMR (ex-GCAM)             |  | CST          |  | Ministry of Islamic Affairs|
| - Audiovisual Media License|  | - OTT Portal |  | - Strict Mosque Broadcast  |
| - Content censorship /     |  |   Registr-   |  |   Directives (Salah/Adhan  |
|   Shari'ah alignment       |  |   ation      |  |   prohibitions)            |
+----------------------------+  +--------------+  +----------------------------+
```
...
+----------------------------------------------------------------------------------------------|--+
                                                                                               |
+----------------------------------------------------------------------------------------------|--+
|                                     SELF-HOSTED CORE PROCESSING STAGE                        |
|  +----------------------------------------------------------------------------------------+  |  |
|  | Bare-Metal Server (Dell PowerEdge R760)                                                |  |  |
|  |                                                                                        |  |  |
|  |  +-----------------------+   RTMP Ingest   +---------------------+                     | <+  |
|  |  | SRS (Simple Realtime  | ==============> | FFmpeg Pipeline     |                     |     |
|  |  | Server) Core Engine   |                 | (Nvidia L4 Trans)   |                     |     |
|  |  +-----------------------+                 +---------------------+                     |     |
|  |             ||                                       ||                                |     |
|  |             || Real-time WebRTC Playback             || Generate ABR HLS               |     |
|  |             \/                                       \/                                |     |
|  |  +-----------------------+                 +---------------------+                     |     |
|  |  | Mobile/Web Clients    |                 | RAM Disk (/tmpfs)   |                     |     |
|  |  | (150ms Ultra-Low Lat) |                 | (.m3u8 &.ts write)  |                     |     |
|  |  +-----------------------+                 +---------------------+                     |     |
|  |                                                      ||                                |     |
|  |                                                      \/ Write to local SAN             |     |
|  |                                            +---------------------+                     |     |
...
|                                     EDGE DELIVERY STAGE                                      |
|  +-------------------------+      HLS / FLV Playback   +----------------------------------+  |  |
|  | Mobile Playback Client  | <======================== | Nginx Edge Caching Servers       | <+  |
|  | (Dynamic ABR Buffering) |                           | (Serving from RAM cache)         |     |
|  +-------------------------+                           +----------------------------------+     |
+-------------------------------------------------------------------------------------------------+
```
...
- **Scenario A (Al Khobar Pilot - 100 CCU):** The physical upload speed required on the server-side is calculated as: 3.0 Mbps x 100 viewers x 1.2 = 360 Mbps.
...
+--------------------------+
             |
             | Batch Ingest (Local Upload)
             v
+--------------------------+
| Persistent RAID Array    | (High-speed NVMe storage)
+--------------------------+
             |
             | Execute Local FFmpeg Script
             v
+--------------------------+
| FFmpeg Pipeline with     | - Transcodes raw files using 'h264_nvenc' acceleration
| NVENC Acceleration       | - Outputs multi-bitrate ABR directory (1080p to 360p)
+--------------------------+
             |
             | Write Standardized Manifests
             v
+--------------------------+
| Local Nginx Edge Cache   | ====> Users (Playable via Flutter App)
+--------------------------+
             |
             | DB Registration
             v
+--------------------------+
| PostgreSQL Catalog       | - Registers Title, Speaker, Date, and Local HLS URL
+--------------------------+
```
...
```
+-------------------------------------------------------------------------------------------------+
|                                     LOCAL INGEST & TRANSCODE STAGE                              |
|  +--------------------+       RTMP/SRT       +--------------------+     Fast tmpfs RAM disk     |
|  | PTZ Video Cameras  | ===================> | Dell R760 Server   | =========================+  |
|  | (Al Khobar Venue)  |                      | (Nvidia L4 Trans)  |                          |  |
|  +--------------------+                      +--------------------+                          |  |
+----------------------------------------------------------------------------------------------|--+
                                                                                               |
+----------------------------------------------------------------------------------------------|--+
|                                     CLOUD STORAGE & EDGE DISTRIBUTION STAGE                  |
|  +-------------------------+     CloudFront Pull     +------------------------------------+  |  |
|  | AWS Jeddah Edge Node    | <====================== | Amazon S3 Storage Bucket           | <+  |
|  | (Middle East Delivery)  |                         | (me-central-2 Riyadh Region)       |     |
|  +-------------------------+                         +------------------------------------+     |
|              ||                                                                                 |
|              || Cached HLS Playback                                                             |
|              \/                                                                                 |
...
+--------------------------+
             |
             | Local Batch Transcode
             v
+--------------------------+
| Dell R760 Local Server   | - Transcodes files via FFmpeg with 'h264_nvenc'
| with Nvidia L4 GPU       | - Standardizes into ABR HLS playlists (1080p to 360p)
+--------------------------+
             |
             | Automated Cloud Upload
             v
+--------------------------+
| Amazon S3 Riyadh Bucket  | (me-central-2 Region) 
+--------------------------+
             |
             | Serve Cached Segments
             v
+--------------------------+
| CloudFront Edge Nodes    | ====> Playback on Users' Flutter App [2, 1]
+--------------------------+
             |
             | Catalog Metadata
             v
+--------------------------+
| PostgreSQL database      | - Records Video Title, Speaker, Date, and S3 URL
+--------------------------+
```
...
+-------------------------------------------------------------------------------------------------+
|                                     LOCAL INGESTION STAGE                                       |
|  +--------------------+       RTMP/RTMPS     +--------------------+      Baity Broadband        |
|  | PTZ Video Cameras  | ===================> | OBS Studio / vMix  | =========================+  |
|  | (Al Khobar Venue)  |                      | (Software Encoder) |                          |  |
|  +--------------------+                      +--------------------+                          |  |
+----------------------------------------------------------------------------------------------|--+
                                                                                               |
+----------------------------------------------------------------------------------------------|--+
|                                     EXTERNAL PROCESSING STAGE (YOUTUBE HACK)                 |
|  +-------------------------+    ABR Transcoding      +------------------------------------+  |  |
|  | YouTube Ingest Nodes    | ======================> | YouTube Live Engine                | <+  |
|  | (Free Processing)       |                         | (Unlisted Privacy Protocol)        |     |
|  +-------------------------+                         +------------------------------------+     |
|                                                                  ||                             |
|                                                                  || Free Automated Recording    |
|                                                                  \/                             |
|  +-------------------------+                         +------------------------------------+     |
|  | YouTube VOD Archives    | <====================== | Automated Video Logging            |     |
|  | (Unlimited Free Storage) |                         | (Instant Stream-to-VOD Archive)    |     |
|  +-------------------------+                         +------------------------------------+     |
+----------------------------------------------------------------------------------------------|--+
                                                                                               |
+----------------------------------------------------------------------------------------------|--+
|                                     HYBRID FRONTEND DELIVERY STAGE                           |
|  +-------------------------+    Inline Web iFrame    +------------------------------------+  |  |
|  | Mobile Playback Client  | <====================== | youtube_player_flutter             | <+  |
|  | (Flutter Player View)   |                         | (WebView Controller Engine)        |     |
|  +-------------------------+                         +------------------------------------+     |
+-------------------------------------------------------------------------------------------------+
```
...
| YouTube Live API   | =================================> | PocketBase BaaS   |
| (Stream Finishes)  |                                    | (Store Video ID)  |
+--------------------+                                    +-------------------+
                                                                    ||
                                                                    || Sync Metadata
                                                                    \/
+--------------------+      Query Video Catalog Logs      +-------------------+
| Flutter Mobile App | <================================= | SQLite Database   |
| (Embedded Player)  |                                    | (Unified Playback)|
+--------------------+                                    +-------------------+
```
...
```
                     ...
                     |  Single Go Binary / Embedded SQLite   |
                     |                                       |
                     |  +--------------+   +--------------+  |
                     |  | Auth Engine  |   | SQLite DB    |  |
                     |  | (Keycloak Alt|   | (Dynamo Alt) |  |
                     |  +--------------+   +--------------+  |
                     |  | Local File   |   | SSE Realtime |  |
                     |  | (S3 Alt)     |   | (Pusher Alt) |  |
                     |  +--------------+   +--------------+  |
                     +---------------------------------------+
                                         ^
                     ...
                     |  Handles 100K+ simultaneous events    |
                     +---------------------------------------+
```
...
```
+--------------------------+
| Existing Video Logs      | (Pre-recorded MP4/MOV files)
+--------------------------+
             |
             | Local Python Script (Bulk Upload)
             v
+--------------------------+
| YouTube Data API         | - Uploads videos as 'Unlisted' to our channel
| (Direct Ingest)          | - YouTube transcodes into ABR formats for free
+--------------------------+
             |
             | Extract Video Meta
             v
+--------------------------+
| PocketBase BaaS          | - Stores Video ID, Title, Speaker, and Date
| (Local SQLite DB)        | - Syncs playback link to user video feeds
+--------------------------+
```
...
2. **Automatic Transcoding:** YouTube receives the files, transcodes them into standard web formats for free, and generates a unique Video ID.
    
3. **Database Indexing:** Our script captures the returned Video ID and writes a new metadata row (Title, Speaker, Date, and Video ID) directly to our local PocketBase SQLite database.
    
4. **App Playback:** When a user opens the video library inside the Flutter app, the player fetches the Video ID from PocketBase and plays the video inline via the embedded player.
