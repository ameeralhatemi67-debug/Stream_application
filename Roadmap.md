...
| P6.1-P6.3 Chat and moderation | Partial | Server rate limits, slow mode, report validation and viewer blocks have local evidence. Client blocks are now server-owned with failure states and refresh-based sync (`bb285c8`); two-device acceptance remains. Real-session UI run (2026-09-23): block, cross-device and unblock sync passed; history-order and paused-room defects fixed. Real-device owner steps remain. | 1.0.6 |
