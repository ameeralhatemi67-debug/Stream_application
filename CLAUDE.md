...
- **v1.1 iOS Integration** (not started) is where iOS gets this same phone-streaming capability, per the roadmap's explicit platform-order decision (Android first, iOS bundled into its own version later) — not a v0.7 gap.
  - **👉 Next Up**: **v0.8 Admin Upgrade** from `doc/Roadmap/v0.8_Admin_Upgrade.md` — the tiered Master Admin / Admin / Permitted Admin (Org Owner & Co-Owner) hierarchy, with checkbox-style granular permission grants. Checkpoint 1 (Roles & Permissions Data Model) is complete: `user_roles`/`user_permissions` RLS hardened to a real tier hierarchy, `has_permission()` RPC added, and `org_owner` auto-derives from `organizations.owner_profile_id` (see ADR-007 in `Core_files/decisions.md`). `org_co_owner` intentionally does not auto-derive yet — no co-owner concept exists in the schema/app; Checkpoint 2/3 need to design that before it can. Checkpoint 2 (Admin Hub Rebuilt on Real Backend) is complete: `AppProvider.isMasterAdmin` distinguishes Master Admin from plain Admin, the verification queue has multi-select batch approve/reject, and a Master-Admin-only "Roles & Permissions" tab grants/revokes Admin/Master Admin and toggles capability checkboxes (`org_owner`/`org_co_owner` stay auto-derive-only, not grantable there). Checkpoint 3 (Org-Scoped Tools for Permitted Admins) is complete: a new `/org-admin` route + `OrgAdminScreen` gives Org Owners/Co-Owners a scoped `OrgManagementView` pinned to `AppProvider.permittedAdminOrgIds`, with `ensureOrgDataLoaded` fixed to seed real orgs missing from the in-memory `_streamers` mock list. Next: Checkpoint 4 (Platform-Wide Chat Moderation Tools).
