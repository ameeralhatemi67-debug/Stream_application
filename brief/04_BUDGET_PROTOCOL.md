...
3. If at the stop `resets_in_min > 75`, don't wait — go to E (the owner resumes with `brief/archive/prompts/RESUME_PROMPT.md`).
...
Then write **`brief/archive/reports/2026-09-22/OVERRUN_REPORT.md`** (budget-driven stop) or **`brief/FINAL_REPORT.md`** (all done). The overrun report must explain *why* — with numbers:
...
5. Safe-state proof (commands + outputs) and what the owner should run next (`brief/archive/prompts/RESUME_PROMPT.md`, split or raise the cap).
...
2. **Prompt.** Paste `brief/archive/prompts/00_MASTER_PROMPT_ASTRA.md` (slim on purpose: OpenAI advises against long scaffolding for Astra). `AGENTS.md` is read by Codex automatically: do not repeat it.
3. **Plan.** Default for Astra is **SPLIT**: window 1 cap 70 (soft 64), window 2 cap 60 (soft 54), never a third; SINGLE (cap 80, one window) if you prefer. No nap loop: each nap re-sends the whole context, which is exactly what burns quota, and the cache lives ~30 minutes anyway. At the window-1 stop it writes the RESUME block and quits. After the reset, open a **fresh** session (not `codex resume`, which replays the whole history) and paste `brief/archive/prompts/RESUME_PROMPT.md` with `MODE: CONTINUE_SPLIT`: the window counter is kept in `brief/.runtime/budget_state.json`, so the meter reports `window=2, cap=60`. The old "send it with an hour left" advice is about Claude's 1-hour cache and does not apply here; pasting with an hour left only means window 1 spends what remains of it.
...
6. Observed at the start of split window 2 (2026-09-20): the meter again returned UNKNOWN (stale log) in a fresh session and Astra recovered from the Codex usage tool with `--live-*`. Treat the Codex display as the primary source and the log file as a lagging cross-check.
7. Harness flag: pass `--source codex` under Codex and `--source claude` under Claude Code on every meter call. With `auto` the meter takes the freshest of the two sources, so a Claude Code session that ended minutes ago could be read as Codex's usage (or the reverse).

## J. Owner-authorized Codex weekly-only run (2026-09-23)

For Codex only, the owner supersedes the historical five-hour plans and 90% weekly guard: work efficiently toward completion, with total weekly usage never exceeding 5%, preferably stopping below it. This is an absolute 5% ceiling, not five extra points. Entry usage was 1%; implementation entry was 2%. Use `node brief/tools/budget_check.mjs --source codex --plan weekly --cap 5` at checkpoints and before broad verification. Weekly 4% is the soft stop, reserving one point for verification and closeout; 5% is STOP. Unknown/stale readings remain fail-closed. Do not wait for or cross a weekly reset to gain budget. No manual runtime edits are needed. The owner authorized the necessary protocol and meter correction in this session. Earlier no-subagent advice is superseded by the owner's explicit request for one consolidated GPT-6 Luna Medium verification after code stabilizes.

## K. Owner override for Claude Opus 5.5 (2026-09-23)

The owner explicitly waives usage metering and percentage caps for Claude Opus 5.5 work on this project until they change this instruction. An absent `brief/.runtime/usage_snapshot.json`, `BUDGET status=UNKNOWN`, `5h n/a`, or a missing status line is not a reason to stop Opus work. Do not call `budget_check.mjs`, wait for a usage reset, invent a reading, or edit meter/runtime files to manufacture one. Do not apply §J's Codex 5% cap to Claude. The older Claude meter steps in §§A–E and §I are historical for Opus while this override is active.

Work efficiently: use focused tests during edits, run each broad suite once at a stable checkpoint, keep logs concise, and avoid repeated full-file reads. The technical checkpoint still requires relevant passing tests, analyzer and gates with honest evidence limits. Launching with `brief/claude_settings.json` remains useful for its safety permissions, but its status-line command is optional for this Opus run. Preserve owner edits and stashes; no push, linked/production Supabase, deployment, secret access or destructive reset is authorized by this override.
