...
**AUTO** (default in the prompt header), decided in Step 0: if `cache_ttl=1h` **and** `resets_in_min ≤ 90` ⇒ SPLIT (window 1 is too short for this job); otherwise SINGLE. Log the choice and the numbers. You may switch SINGLE→SPLIT at a phase boundary only if the tail of 03 clearly can't fit and the cache is 1h; never switch to gain room mid-step.

## Step 0 (first actions, ≤5 tool calls)
1. `node brief/tools/budget_check.mjs --plan single`. `UNKNOWN`: make one cheap call (`date`) and retry once. Still unknown ⇒ **stop now** and tell the owner to do section A (cost ≈ nothing; fail closed).
2. Choose the plan (B). From now on pass `--plan <plan>` on every check.
...
3. **Plan.** Always SINGLE (the tool forces it). No nap loop: each nap re-sends the whole context, which is exactly what burns quota, and the cache lives ~30 minutes anyway. When the window ends: safe stop, RESUME block, and in the next window start a **fresh session** with `brief/RESUME_PROMPT.md` (not `codex resume`, which replays the whole history).
