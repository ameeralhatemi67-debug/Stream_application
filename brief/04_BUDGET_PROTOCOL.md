...
## J. Owner-authorized weekly-only run (2026-09-23)

For this continuation, the owner supersedes the historical five-hour plans and 90% weekly guard: work efficiently toward completion, with total weekly usage never exceeding 5%, preferably stopping below it. This is an absolute 5% ceiling, not five extra points. Entry usage was 1%; implementation entry was 2%. Use `node brief/tools/budget_check.mjs --source codex --plan weekly --cap 5` at checkpoints and before broad verification. Weekly 4% is the soft stop, reserving one point for verification and closeout; 5% is STOP. Unknown/stale readings remain fail-closed. Do not wait for or cross a weekly reset to gain budget. No manual runtime edits are needed. The owner authorized the necessary protocol and meter correction in this session. Earlier no-subagent advice is superseded by the owner's explicit request for one consolidated GPT-6 Luna Medium verification after code stabilizes.
