...
1. `git stash list` shows two owner stashes. Do NOT apply, pop or drop either: the owner has already restored the five brief/meter files into the working tree (they show as modified) and added `brief/BONUS_PROMPT.md`. Commit exactly these by explicit path: `brief/00_MASTER_PROMPT_ASTRA.md brief/04_BUDGET_PROTOCOL.md brief/DESIGN_PROMPT.md brief/RESUME_PROMPT.md brief/tools/budget_check.mjs brief/BONUS_PROMPT.md` with message "docs: restore split-meter updates and add bonus plan".
2. `node brief/tools/budget_check.mjs --plan bonus --bonus-start` (this once). It records the used % now (S) and sets the bonus cap = min(94, S + 40), soft = cap - 6. Afterwards run `node brief/tools/budget_check.mjs --plan bonus` (no flag) before each item and every 10-15 tool calls. If Codex's live usage is higher than the file snapshot, use the higher number. Never `--new-run`; never edit the meter or its caps.
